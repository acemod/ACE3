    ...
    class ACE_RscHUDHelper {
        class controls {
            class MY_HUD_RscPicture: RscPicture { // Classname is also used as uiNamespace variable name.
                onLoad = QUOTE(_this call EFUNC(ui,registerHUDElement)); // Register the control as a HUD element to be hidden on pause/featureCamera
                onUnload = QUOTE(_this call EFUNC(ui,removeHUDElement)); // Remove the control's associated variables when it is destroyed.
                ...
                // Rest of UI properties as normal
            };
            class MY_HUD_ControlGroup: RscControlsGroupNoScrollBars {
                idc = -1; // Recommended for compatibility. Access control group by using 'uiNamespace getVariable ["MY_HUD_ControlGroup", controlNull];
                ...
                class controls { // Sub-controls aren't registered as variables. Assign manually with onLoad event or use allControls command.
