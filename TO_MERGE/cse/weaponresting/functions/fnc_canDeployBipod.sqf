...
if ((call cse_fnc_hasBipod_WR) && _weapon == primaryWeapon player && !(weaponLowered player) && _playerAnimationState in ALLOWED_ANIMATION_STATES) then {
	...
	_isLongRifle = [configFile >> "CfgWeapons" >> _weapon, "Rifle_Long_Base_F"] call EFUNC(common,inheritsFrom);
	...
	_canDeployBipod = [_bipodTop, _bipodBottom, 0.05] call EFUNC(common,getFirstObjectIntersection);
	if (_canDeployBipod select 0) exitWith {};
	_canDeployBipod = [_bipodTop, _bipodBottom, 0.05] call EFUNC(common,getFirstTerrainIntersection);
	if (_canDeployBipod select 0) exitWith {};
};
