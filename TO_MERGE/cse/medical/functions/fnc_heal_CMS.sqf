...
if !((CSE_AID_KIT_RESTRICTIONS_CMS == 0 && ([_caller] call FUNC(inMedicalFacility_CMS))) || (CSE_AID_KIT_RESTRICTIONS_CMS == 1 && ([_caller] call FUNC(inMedicalFacility_CMS)) && (!([_unit] call FUNC(hasOpenWounds_CMS)))) || (CSE_AID_KIT_RESTRICTIONS_CMS == 2) || (CSE_AID_KIT_RESTRICTIONS_CMS == 3 && (!([_unit] call FUNC(hasOpenWounds_CMS))))) exitwith {};
...
if (!(_unit getvariable ["cms_isDead",false]) && alive _unit) then {
	[[_unit,_caller], "FUNC(healLocal_CMS)", _unit, false] spawn BIS_fnc_MP;
};
