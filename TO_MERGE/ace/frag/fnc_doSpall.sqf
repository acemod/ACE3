	...
	if(!_exit) then {
		...
		if(!isNil "_spallPos") then {
			...
			if(_explosive > 0) then {
				...
				if(_warn) then {
					diag_log text format["Ammo class %1 lacks proper explosive properties definitions for sys_frag!", _roundType]; //TODO: turn this off when we get closer to release
				};
