...
if (GVAR(plottingBoard_Shown) == 0) then {
    ...
    if (!("ACE_PlottingBoard" in (ACE_player call EFUNC(common,uniqueItems)))) exitWith {};
...
if ((GVAR(mapTool_Shown) > 0) && {"ACE_MapTools" in (ACE_player call EFUNC(common,uniqueItems))}) then {
