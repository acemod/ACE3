...
if (toLower _itemName in ["b_ir_grenade", "o_ir_grenade", "i_ir_grenade"]) then {
    ...
    if ((["ACE_chemlights"] call EFUNC(common,isModLoaded)) && {typeOf _attachedObject isKindOf ["Chemlight_base", configFile >> "CfgAmmo"]} && {[_attachedObject] call EFUNC(chemlights,isIRClass)}) then {
        [objNull, _attachToVehicle getVariable [QEGVAR(chemlights,IRAttach), objNull]] call EFUNC(chemlights,removeIR);
        _attachToVehicle setVariable [QEGVAR(chemlights,IRAttach), objNull];
    };
