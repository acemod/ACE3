class GVAR(medicalMenu) {
	idd = 314412;
	movingEnable = true;
	onLoad = QUOTE(uiNamespace setVariable [ARR_2(QUOTE(QGVAR(medicalMenu)), _this select 0)]; [ARR_2(QUOTE(QGVAR(id)), true)] call EFUNC(gui,blurScreen); [_this select 0] call FUNC(onMenuOpen_CMS););
	...
	// center
		...
		class selectHead: ACE_gui_buttonBase {
			...
			animTexturePressed = "#(argb,8,8,3)color(0,0,0,0.0)";
			animTextureDefault = "#(argb,8,8,3)color(0,0,0,0.0)";
			action = QUOTE(GVAR(selectedBodyPart) = 'head'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
		class selectTorso : selectHead {
			...
			action = QUOTE(GVAR(selectedBodyPart) = 'body'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
		class selectLeftArm: selectHead{
			...
			action = QUOTE(GVAR(selectedBodyPart) = 'hand_r'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
		class selectRightArm: selectLeftArm{
			...
			action = QUOTE(GVAR(selectedBodyPart) = 'hand_l'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
		class selectLeftLeg :selectHead {
			...
			action = QUOTE(GVAR(selectedBodyPart) = 'leg_r'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
		class selectRightLeg :selectLeftLeg {
			...
			action = QUOTE(GVAR(selectedBodyPart) = 'leg_l'; [GVAR(INTERACTION_TARGET)] call FUNC(updateUIInfo_CMS););
		};
