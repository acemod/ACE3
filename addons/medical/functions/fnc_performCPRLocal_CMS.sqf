...
[_this, QUOTE(FUNC(performCPRProvider_CMS)), _caller, false] call BIS_fnc_MP;
_n = _unit getvariable [QEGVAR(common,ENABLE_REVIVE_COUNTER),0];
if (_n > 0) then {
	_n = _n - random(20);
	if (_n < 0) then {
		_n = 0;
	};
	_unit setvariable [QEGVAR(common,ENABLE_REVIVE_COUNTER), _n];
};