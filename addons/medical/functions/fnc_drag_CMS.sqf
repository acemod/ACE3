...
if (!([_caller] call EFUNC(common,canInteract)) || {_caller == _unit} || {(([_unit] call EFUNC(common,isAwake)))}) exitwith { ["FUNC(drag_CMS) call failed condition: canInteract, equal, is awake"] call EFUNC(common,debug);
