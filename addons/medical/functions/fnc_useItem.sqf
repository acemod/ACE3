...
if (GVAR(setting_allowSharedEquipment) && {[_patient, _item] call EFUNC(common,hasItem)}) exitWith {
    if (local _patient) then {
        ["useItem", [_patient, _item]] call CBA_fnc_localEvent;
    } else {
        ["useItem", [_patient, _item], _patient] call CBA_fnc_targetEvent;
    };
    [true, _patient];
};
...
if ([_medic, _item] call EFUNC(common,hasItem)) exitWith {
    if (local _medic) then {
        ["useItem", [_medic, _item]] call CBA_fnc_localEvent;
    } else {
        ["useItem", [_medic, _item], _medic] call CBA_fnc_targetEvent;
    };
    [true, _medic];
};
...
if ([vehicle _medic] call FUNC(isMedicalVehicle) && {vehicle _medic != _medic}) then {
    _crew = crew vehicle _medic;
    {
        if ([_medic, _x] call FUNC(canAccessMedicalEquipment) && {([_x, _item] call EFUNC(common,hasItem))}) exitWith {
            _return = [true, _x];
            if (local _x) then {
                ["useItem", [_x, _item]] call CBA_fnc_localEvent;
            } else {
                ["useItem", [_x, _item], _x] call CBA_fnc_targetEvent;
            };
        };
    } forEach _crew;
};
