...
#define TREATMENT_IV(ITEM) QUOTE([ARR_4(_this select 0,_this select 1,call FUNC(getSelectedBodyPart_CMS),ITEM)] call FUNC(iv_CMS))
...
if ((GVAR(INTERACTION_TARGET) != player && (isNull ([player] call EFUNC(common,getCarriedObj))))) then {
	...
	if ([player,GVAR(INTERACTION_TARGET), 'ACE_blood_iv']  call FUNC(hasEquipment_CMS)) then {
		_return pushback [localize "STR_ACE_ACTION_BLOODIV_1000ml", TREATMENT_IV('ACE_blood_iv'),localize "STR_ACE_ACTION_BLOODIV_1000ML_TOOLTIP"];
	};
	if ([player,GVAR(INTERACTION_TARGET), 'ACE_blood_iv_500']  call FUNC(hasEquipment_CMS)) then {
		_return pushback [localize "STR_ACE_ACTION_BLOODIV_500ml", TREATMENT_IV('ACE_blood_iv_500'),localize "STR_ACE_ACTION_BLOODIV_500ML_TOOLTIP"];
	};
	if (([player,GVAR(INTERACTION_TARGET),'ACE_blood_iv_250']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_BLOODIV_250ml", TREATMENT_IV('ACE_blood_iv_250'),localize "STR_ACE_ACTION_BLOODIV_250ML_TOOLTIP"];
	};

	if (([player,GVAR(INTERACTION_TARGET),'ACE_plasma_iv']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_PLASMAIV_1000ml", TREATMENT_IV('ACE_plasma_iv'),localize "STR_ACE_ACTION_PLASMAIV_1000ML_TOOLTIP"];
	};
	if (([player,GVAR(INTERACTION_TARGET),'ACE_plasma_iv_500']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_PLASMAIV_500ml", TREATMENT_IV('ACE_plasma_iv_500'),localize "STR_ACE_ACTION_PLASMAIV_500ML_TOOLTIP"];
	};
	if (([player,GVAR(INTERACTION_TARGET),'ACE_plasma_iv_250']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_PLASMAIV_250ml", TREATMENT_IV('ACE_plasma_iv_250'),localize "STR_ACE_ACTION_PLASMAIV_250ML_TOOLTIP"];
	};

	if (([player,GVAR(INTERACTION_TARGET),'ACE_saline_iv']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_SALINEIV_1000ml", TREATMENT_IV('ACE_saline_iv'),localize "STR_ACE_ACTION_SALINEIV_1000ML_TOOLTIP"];
	};
	if (([player,GVAR(INTERACTION_TARGET),'ACE_saline_iv_500']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_SALINEIV_500ml", TREATMENT_IV('ACE_saline_iv_500'),localize "STR_ACE_ACTION_SALINEIV_500ML_TOOLTIP"];
	};
	if (([player,GVAR(INTERACTION_TARGET),'ACE_saline_iv_250']  call FUNC(hasEquipment_CMS))) then {
		_return pushback [localize "STR_ACE_ACTION_SALINEIV_250ml", TREATMENT_IV('ACE_saline_iv_250'),localize "STR_ACE_ACTION_SALINEIV_250ML_TOOLTIP"];
	};
