...
if (getNumber (_config >> QGVAR(canDrag)) == 1) then {
    ...
    private _ignoreWeight = parseNumber getNumber (_config >> QGVAR(ignoreWeight));

    [_object, true, _position, _direction, _ignoreWeight] call FUNC(setDraggable);
};
...
if (getNumber (_config >> QGVAR(canCarry)) == 1) then {
    ...
    private _ignoreWeight = parseNumber getNumber (_config >> QGVAR(ignoreWeightCarry));

    [_object, true, _position, _direction, _ignoreWeight] call FUNC(setCarryable);
};
