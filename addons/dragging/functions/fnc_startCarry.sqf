 ...
 *
 * Example:
 * [player, cursorObject] call ace_dragging_fnc_startCarry;
 *
 * Public: No
 */
...
if (_target isKindOf "CAManBase") then {
    ...
    // Get last weapon index from unit
    private _lastWeaponIndex = ((_unit weaponsInfo [""]) apply {_x select 0}) select -1;

    // Select a weapon index that is not used by any weapons
    _unit action ["SwitchWeapon", _unit, _unit, _lastWeaponIndex + 1];
