...
private _change = (if (_isInSmoke) then {
    EFFECT_RATE * GVAR(smoke_effectFactor)
} else {
    RECOVERY_RATE * GVAR(smoke_recoveryFactor)
}) * PFH_INTERVAL;

private _fnc_updateLevel = {
    params ["_level", "_protect"];
    private _innerChange = if (_protect == 1 && _isInSmoke) then {
        RECOVERY_RATE * GVAR(smoke_recoveryFactor) * PFH_INTERVAL
    } else {
        _change
    };
...
// Breathing
private _levelBreathing = _unit getVariable [QGVAR(smoke_breathingLevel), 0];
...
_unit setVariable [QGVAR(smoke_breathingLevel), _levelBreathing];
