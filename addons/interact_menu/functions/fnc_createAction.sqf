 ...
 * 11: Icon color (Hex or RGB) <STRING> or <ARRAY> (Optional)
 *
 * Return value:
 * Action <ARRAY>
 *
...
private ["_insertChildren","_customParams","_position","_distance","_params", "_modifierFunction", "_iconColor"];
...
_iconColor = if (count _this > 11) then {
    if (typeName (_this select 11) == "ARRAY") then {
        (_this select 11) call BIS_fnc_colorRGBtoHTML
    } else {
        _this select 11
    };
} else {
