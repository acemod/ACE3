...
["playerInventoryChanged",  {_this call FUNC(updatePPEffects)}] call CFUNC(addEventHandler);
...
["playerVehicleChanged",    {_this call FUNC(updatePPEffects)}] call CFUNC(addEventHandler);
["playerTurretChanged",     {_this call FUNC(updatePPEffects)}] call CFUNC(addEventHandler);

// Add keybinds
["ACE3 Equipment", QGVAR(IncreaseNVGBrightness), localize LSTRING(IncreaseNVGBrightness),
{
    // Conditions: canInteract
    if !([ACE_player, objNull, ["isNotEscorting", "isNotInside", "isNotSitting"]] call CFUNC(canInteractWith)) exitWith {false};
    // Conditions: specific
...
["firedPlayer", DFUNC(blending)] call CFUNC(addEventHandler);
["firedPlayerVehicle", DFUNC(blending)] call CFUNC(addEventHandler);
