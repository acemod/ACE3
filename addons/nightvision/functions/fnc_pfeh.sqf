...
if (CBA_missionTime < GVAR(nextEffectsUpdate)) then {
    ...
    if ((cameraView == "GUNNER") && {[ACE_player] call CBA_fnc_canUseWeapon}) then {
        ...
        if (count ("'NVG' in getArray (_x >> 'visionMode')" configClasses (configFile >> "CfgWeapons" >> ((ACE_player weaponAccessories currentWeapon ACE_player) select 2) >> "ItemInfo" >> "OpticsModes")) > 0) exitWith {}; // NVS are good
