...
if (CBA_missionTime < GVAR(nextEffectsUpdate)) then {
    ...
    if ((cameraView == "GUNNER") && {[ACE_player] call CBA_fnc_canUseWeapon}) then {
        ...
        if (currentWeapon ACE_player == primaryWeapon ACE_player && (primaryWeaponItems ACE_player) select 2 == "optic_NVS") exitWith {_blurFinal = _blurFinal * linearConversion [0, 1, GVAR(aimDownSightsBlur), 1, ST_NVG_CAMERA_BLUR_SIGHTS_NVS]}; // NVS are good
