...
[
    ...
    [_unit, _target, _fuel],
    {
        ...
        if (_fuel > 0 ) then {
            ["displayTextStructured", _unit, [[LSTRING(Hint_RemainingFuel), _fuel], 2, _unit]] call CFUNC(objectEvent);
        } else {
            ["displayTextStructured", _unit, [LSTRING(Hint_Empty), 2, _unit]] call CFUNC(objectEvent);
        };
        true
    },
    {true},
