class EGVAR(arsenal,stats) {
    ...
    class GVAR(minedetectorRadius): statBase {
        ...
        showText = 1;
        textStatement = QUOTE(private _radius = getNumber (configFile >> 'ACE_detector' >> 'detectors' >> (configName (_this select 1)) >> (_this select 0 select 0)); format [ARR_3('%1m (%2ft)',_radius,(_radius / 0.3048) toFixed 1)]);
