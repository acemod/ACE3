...
// Add keybinds
["localize ELSTRING(common,ACEConfigCategoryCommon)", QGVAR(openDoor), localize LSTRING(OpenDoor),
{
...
["localize ELSTRING(common,ACEConfigCategoryCommon)", QGVAR(tapShoulder), localize LSTRING(TapShoulder),
{
    // Conditions: canInteract
    if !([ACE_player, objNull, []] call EFUNC(common,canInteractWith)) exitWith {false};
    // Conditions: specific
...
["localize ELSTRING(common,ACEConfigCategoryCommon)", QGVAR(modifierKey), localize LSTRING(ModifierKey),
{
    // Conditions: canInteract
