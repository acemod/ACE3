...
if(local _lamp) then {
    [_eventName, [_lamp, _hitPointsDamage, DISABLED_LAMP_DMG]] call CFUNC(localEvent);
} else {
    [_eventName, [_lamp], [_lamp, _hitPointsDamage, DISABLED_LAMP_DMG]] call CFUNC(targetEvent);
};
