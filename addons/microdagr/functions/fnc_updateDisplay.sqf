...
case (APP_MODE_INFODISPLAY): {
        ...
        if (GVAR(currentWaypoint) == -1) then {
            ...
            if (!(_targetPosLocationASL isEqualTo [])) then {
                ...
                _rangeText = format ["%1km", _2dDistanceKm toFixed (GVAR(waypointPrecision) +1)];
...
case (APP_MODE_WAYPOINTS): {
        ...
        lbClear _wpListBox;
        {
            ...
            _wpListBox lbSetTextRight [_forEachIndex, (format ["%1km", _2dDistanceKm toFixed (GVAR(waypointPrecision) +1)])];
        } forEach _waypoints;
