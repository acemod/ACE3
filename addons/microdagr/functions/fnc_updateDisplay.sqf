...
case (APP_MODE_INFODISPLAY): {
        ...
        if (GVAR(currentWaypoint) == -1) then {
            ...
            if (!(_targetPosLocationASL isEqualTo [])) then {
                ...
                _rangeText = format ["%1km", ([_2dDistanceKm, 1, 2] call CBA_fnc_formatNumber)];
...
case (APP_MODE_WAYPOINTS): {
        ...
        lbClear _wpListBox;
        {
            ...
            _wpListBox lbSetTextRight [_forEachIndex, (format ["%1km", ([_2dDistanceKm, 1, 2] call CBA_fnc_formatNumber)])];
        } forEach _waypoints;
