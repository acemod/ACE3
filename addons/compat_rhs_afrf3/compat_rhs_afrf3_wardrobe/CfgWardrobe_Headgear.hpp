//Pilot Helmets with Visors
// ToDo use MACROs
class rhs_zsh7a_mike: EGVAR(wardrobe,base_H_visor_up) {
    class modifiableTo { class rhs_zsh7a_mike_alt; };
    components[] = {};
};
class rhs_zsh7a_mike_alt: EGVAR(wardrobe,base_H_visor_down) {
    class modifiableTo { class rhs_zsh7a_mike; };
    components[] = {};
};

class rhs_zsh7a_mike_green: EGVAR(wardrobe,base_H_visor_up) {
    class modifiableTo { class rhs_zsh7a_mike_green_alt; };
    components[] = {};
};
class rhs_zsh7a_mike_green_alt: EGVAR(wardrobe,base_H_visor_down) {
    class modifiableTo { class rhs_zsh7a_mike_green; };
    components[] = {};
};

class rhs_zsh7a: EGVAR(wardrobe,base_H_visor_up) {
    class modifiableTo { class rhs_zsh7a_alt; };
    components[] = {};
};
class rhs_zsh7a_alt: EGVAR(wardrobe,base_H_visor_down) {
    class modifiableTo { class rhs_zsh7a; };
    components[] = {};
};

// Tanker Hat
// Todo user EGVAR(wardrobe,base)
class rhs_tsh4: EGVAR(wardrobe,base_H_goggles_off) {
    class modifiableTo { class rhs_tsh4_bala; class rhs_tsh4_ess;};
    components[] = {};
};
class rhs_tsh4_bala: EGVAR(wardrobe,base_H_mask_on) {
    class modifiableTo { class rhs_tsh4; class rhs_tsh4_ess_bala;};
    components[] = {"rhs_balaclava1_olive"};
};
class rhs_tsh4_ess: EGVAR(wardrobe,base_H_goggles_on) {
    class modifiableTo { class rhs_tsh4_ess_bala; class rhs_tsh4;};
    components[] = {"G_Combat"}; //Vanilla Goggles because RHS ARF doesn't have any
};
class rhs_tsh4_ess_bala: EGVAR(wardrobe,base_H_goggles_on) {
    class modifiableTo { class rhs_tsh4_bala; class rhs_tsh4_ess;};
    components[] = {"rhs_balaclava1_olive", "G_Combat"};
};

// components

// rhs_facewear_6m2
// rhs_facewear_6m2_1

// rhs_6m2_nvg
// rhs_6m2_1_nvg

// rhs_balaclava           // 1 big opening
// rhs_balaclava1_olive    // 2 smaller openings

// // Helmet
// // Helmet with Balaclava
// // Helmet with ESS Glasses
// // Helmet with Balaclava and ESS Glasses

// rhs_6b26_green
// rhs_6b26_bala_green
// rhs_6b26_ess_green
// rhs_6b26_ess_bala_green

// rhs_6b26_digi
// rhs_6b26_digi_bala
// rhs_6b26_digi_ess
// rhs_6b26_digi_ess_bala

// rhs_6b26
// rhs_6b26_bala
// rhs_6b26_ess
// rhs_6b26_ess_bala



// rhs_6b27m_green
// rhs_6b27m_green_bala
// rhs_6b27m_green_ess
// rhs_6b27m_green_ess_bala

// rhs_6b27m_digi
// rhs_6b27m_digi_bala
// rhs_6b27m_digi_ess
// rhs_6b27m_digi_ess_bala

// rhs_6b27m
// rhs_6b27m_bala
// rhs_6b27m_ess
// rhs_6b27m_ess_bala

// rhs_6b27m_ml
// rhs_6b27m_ml_bala
// rhs_6b27m_ml_ess
// rhs_6b27m_ML_ess_bala

// rhs_6b28_green
// rhs_6b28_green_bala
// rhs_6b28_green_ess
// rhs_6b28_green_ess_bala

// rhs_6b28
// rhs_6b28_bala
// rhs_6b28_ess
// rhs_6b28_ess_bala


// rhs_6b28_flora
// rhs_6b28_flora_bala
// rhs_6b28_flora_ess
// rhs_6b28_flora_ess_bala

// rhs_6b47
// rhs_6b47_6m2
// rhs_6b47_6m2_1
// rhs_6b47_bala
// rhs_6b47_ess
// rhs_6b47_ess_bala

// rhs_6b7_1m
// rhs_6b7_1m_bala1    // 2 small
// rhs_6b7_1m_bala2    // 1 big


// rhs_6b7_1m_emr
// rhs_6b7_1m_bala1_emr    // 2 small
// rhs_6b7_1m_bala2_emr    // 1 big
// rhs_6b7_1m_emr_ess
// rhs_6b7_1m_emr_ess_bala


// rhs_6b7_1m
// rhs_6b7_1m_bala1        // 2 small
// rhs_6b7_1m_bala2        // 1 big
// rhs_6b7_1m_ess
// rhs_6b7_1m_ess_bala     // 2 small

// rhs_6b7_1m_flora
// rhs_6b7_1m_bala1_flora  // 2 small
// rhs_6b7_1m_bala2_flora  // 1 big
// rhs_6b7_1m_flora_ns3    // Funny glasses

// rhs_6b7_1m_olive
// rhs_6b7_1m_bala1_olive  // 2 small
// rhs_6b7_1m_bala2_olive  // 1 big


// rhs_altyn_novisor
// rhs_altyn_novisor_bala
// rhs_altyn_novisor_ess
// rhs_altyn_novisor_ess_bala

// rhs_altyn
// rhs_altyn_visordown


// // Same beret but slightly different
// rhs_beret_mp1
// rhs_beret_mp2

// rhs_beret_vdv2
// rhs_beret_vdv3


// rhs_fieldcap
// rhs_fieldcap_helm   // rhs_6b27m

// rhs_fieldcap_ml
// rhs_fieldcap_helm_ml    // rhs_6b27m_ml

// rhs_fieldcap_digi
// rhs_fieldcap_helm_digi  // rhs_6b27m_digi


// // Hats and _back version


// rhs_fieldcap_m88
// rhs_fieldcap_m88_back

// rhs_fieldcap_m88_grey
// rhs_fieldcap_m88_grey_back

// rhs_fieldcap_m88_klmk
// rhs_fieldcap_m88_klmk_back

// rhs_fieldcap_m88_moldovan_ttsko_blue
// rhs_fieldcap_m88_moldovan_ttsko_blue_back

// rhs_fieldcap_m88_moldovan_ttsko_cold
// rhs_fieldcap_m88_moldovan_ttsko_cold_back

// rhs_fieldcap_m88_moldovan_ttsko_desat
// rhs_fieldcap_m88_moldovan_ttsko_desat_back

// rhs_fieldcap_m88_moldovan_ttsko_hot
// rhs_fieldcap_m88_moldovan_ttsko_hot_back

// rhs_fieldcap_m88_ttsko_alt
// rhs_fieldcap_m88_ttsko_alt_back

// rhs_fieldcap_m88_ttsko_vdv
// rhs_fieldcap_m88_ttsko_vdv_back

// rhs_fieldcap_m88_ttsko
// rhs_fieldcap_m88_ttsko_back

// rhs_fieldcap_m88_vsr
// rhs_fieldcap_m88_vsr_back

// rhs_fieldcap_m88_vsr_2
// rhs_fieldcap_m88_vsr_2_back

// rhs_fieldcap_m88_woodland
// rhs_fieldcap_m88_woodland_back



// // Visor up / Down

// rhs_zsh7a_mike
// rhs_zsh7a_mike_alt  // Visor Down

// rhs_zsh7a_mike_green
// rhs_zsh7a_mike_green_alt

// rhs_zsh7a
// rhs_zsh7a_alt


