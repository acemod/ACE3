...
if ((player distance _unit ) >= 2700) exitWith {};
if ((side effectiveCommander (vehicle _unit)) != (side player)) then {
    ...
    _projectile = _this select 5;	// The projectile (duh!)
    ...
    _dDist      = 7 + (_hit / 2);					// Calculate the detectionDistance (dDist)
    if ((_dDist) > 28) then {_dDist = 28};
