...
GVAR(TrackerpfID) = [{
    ...
    if (isNull((getPilotCameraTarget _vehicle) select 2) ) then {
        ...
        if(_distance > 0.15) then {
            private _vectorToLaser = (getposASL  _vehicle) vectorFromTo (getPosASL (laserTarget _vehicle));
            private _vectorToSpot = (getposASL  _vehicle) vectorFromTo ((getPilotCameraTarget _vehicle) select 1);
