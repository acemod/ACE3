...
if (!((["ace_cargo"] call EFUNC(common,isModLoaded)) && {missionNamespace getVariable [QEGVAR(cargo,enable), false]})) exitWith {
...
if ((_vehicle getVariable [QEGVAR(cargo,loaded), []]) isEqualTo []) exitWith {
    ["No cargo"] call FUNC(showMessage);
};
...
[_vehicle, {
    ...
    if ((!alive _vehicle) || {!alive _pilot} || {[_pilot] call EFUNC(common,isPlayer)}) exitWith {WARNING_2("vehicle %1 - pilot %2 are invalid",_vehicle,_pilot);};
...
}, localize LSTRING(moduleCargoParadrop_DisplayName)] call FUNC(getModuleDestination);
