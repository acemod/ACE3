...
["ACE3 Weapons", QGVAR(SelectGrenadeFlash), localize LSTRING(SelectGrenadeFlash), {
    // Conditions: canInteract
    if !([ACE_player, ACE_player, ["isNotInside", "isNotEscorting"]] call EFUNC(common,canInteractWith)) exitWith {false};
    // Don't change mode or show hint if advanced throwing is active
    if (ACE_player getVariable [QEGVAR(advanced_throwing,inHand), false]) exitWith {false};
    ...
    // Statement
    [ACE_player, 3] call FUNC(selectNextGrenade);
    true
},
{false},
[0, [false, false, false]], false] call CBA_fnc_addKeybind; //Unbound

["ACE3 Weapons", QGVAR(SelectGrenadeSmoke), localize LSTRING(SelectGrenadeSmoke), {
    // Conditions: canInteract
    if !([ACE_player, ACE_player, ["isNotInside", "isNotEscorting"]] call EFUNC(common,canInteractWith)) exitWith {false};
    // Don't change mode or show hint if advanced throwing is active
    if (ACE_player getVariable [QEGVAR(advanced_throwing,inHand), false]) exitWith {false};
    ...
    // Statement
    [ACE_player, 4] call FUNC(selectNextGrenade);
    true
},
{false},
[0, [false, false, false]], false] call CBA_fnc_addKeybind; //Unbound

["ACE3 Weapons", QGVAR(SelectGrenadeFlare), localize LSTRING(SelectGrenadeFlare), {
    // Conditions: canInteract
    if !([ACE_player, ACE_player, ["isNotInside", "isNotEscorting"]] call EFUNC(common,canInteractWith)) exitWith {false};
    // Don't change mode or show hint if advanced throwing is active
    if (ACE_player getVariable [QEGVAR(advanced_throwing,inHand), false]) exitWith {false};
    ...
    // Statement
    [ACE_player, 5] call FUNC(selectNextGrenade);
    true
},
{false},
[0, [false, false, false]], false] call CBA_fnc_addKeybind; //Unbound

["ACE3 Weapons", QGVAR(SelectGrenadeIncendiary), localize LSTRING(SelectGrenadeIncendiary), {
    // Conditions: canInteract
    if !([ACE_player, ACE_player, ["isNotInside", "isNotEscorting"]] call EFUNC(common,canInteractWith)) exitWith {false};
    // Don't change mode or show hint if advanced throwing is active
    if (ACE_player getVariable [QEGVAR(advanced_throwing,inHand), false]) exitWith {false};
    ...
    // Statement
    [ACE_player, 6] call FUNC(selectNextGrenade);
    true
},
{false},
[0, [false, false, false]], false] call CBA_fnc_addKeybind; //Unbound
