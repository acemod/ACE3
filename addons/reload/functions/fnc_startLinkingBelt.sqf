...
private _condition = {
    ...
    ([_player, _target, []] call CFUNC(canInteractWith)) && ((_player distance _target) < 3) && ((speed _target) < 1)
};
...
private _onFinish = {
    ...
    ["linkedAmmo", [_target], [_target, _player, _magazine]] call CFUNC(targetEvent);
};
...
[_player, _magazineType, _maxAmmo] call CFUNC(removeSpecificMagazine);

// Call progress bar
[4, [_player, _target, [_magazineType, _maxAmmo]], _onFinish, _onFailure, (localize LSTRING(LinkingBelt)), _condition] call CFUNC(progressBar);
