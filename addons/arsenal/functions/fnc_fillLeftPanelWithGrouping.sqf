...
/*
 * Author: ACE Team
 * Populates the arsenal left panel tree with items, supporting different grouping methods or flat display.
 *
 * Arguments:
...
private _groupingEnabled = (GVAR(groupingEnabled) isEqualTo true);
private _groupingMethod = GVAR(groupingMethod);

if (!_groupingEnabled) then {
    // Flat display - just add all items directly to tree without grouping
    {
        ...
        if (!_skip) then {
            private _itemIndex = _treeCtrl tvAdd [[], _displayName];
            _treeCtrl tvSetData [[_itemIndex], _className];
            _treeCtrl tvSetPicture [[_itemIndex], _itemPicture];
            _treeCtrl tvSetTooltip [[_itemIndex], format ["%1\n%2", _displayName, _className]];
            ...
            if (GVAR(enableModIcons) > 0) then {
                ...
                if (_modPicture != "") then {
                    _treeCtrl tvSetPictureRight [[_itemIndex], _modPicture];
                };
            };
            
            // Set favorites color
            if ((toLowerANSI _className) in GVAR(favorites)) then {
                _treeCtrl tvSetPictureColor [[_itemIndex], FAVORITES_COLOR];
            };
        };
    } forEach _items;
} else {
    ...
    
    {
        ...
        if (_groupingMethod == 0) then {
            // Group by Mod/DLC (existing logic)
            ...
            // Group by Class Inheritance (new logic)
