...
private _stateOld = [_unit, _weapon] call EFUNC(common,getWeaponState);

_stateOld params ["_items", "", "_magazines", "_ammo"];
...
// Changing virtual load
[_target, _gunbag, -_mass] call EFUNC(movement,addLoadToUnitContainer);
_gunbag setVariable [QGVAR(gunbagWeapon), _stateOld, true];
