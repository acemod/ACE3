...
if (!alive _target || {!(_target call FUNC(isInjured))} || {GVAR(requireItems) != 0 && {
    ...
    _treatmentEvent in ["#needsBandages", "#needsBandagesOrTourniquets", "#needsIV", "#tooManyMeds", "#needsSlowerHeart", "#needsFasterHeart"]
}}) exitWith {
    _this forceSpeed -1;
