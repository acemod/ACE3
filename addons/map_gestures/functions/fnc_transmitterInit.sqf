...
GVAR(MouseMoveHandlerID) = _mapCtrl ctrlAddEventHandler ["MouseMoving", {
    ...
    if (!GVAR(EnableTransmit) || {missionNamespace getVariable [QEGVAR(maptools,mapTool_isDragging), false]} || {missionNamespace getVariable [QEGVAR(maptools,mapTool_isRotating), false]}) exitWith {};
