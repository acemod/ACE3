...
(_display displayCtrl 632) ctrlAddEventHandler ["LBDrop", {
    ...
    ["DisarmDropItems", [GVAR(disarmTarget)], [ACE_player, GVAR(disarmTarget), [_data]]] call CFUNC(targetEvent);
...
[{
    ...
    if ((!([_player, _target] call FUNC(canPlayerDisarmUnit))) ||
        ...
        _playerName ctrlSetText ([GVAR(disarmTarget), false, true] call CFUNC(getName));
