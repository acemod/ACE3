...
[{[_this] call FUNC(deployRopes)}, _vehicle, _deployTime] call CFUNC(waitAndExecute);
...
DFUNC(deployAIRecursive) = {
    ...
    if !(_unitsToDeploy isEqualTo []) then {
        [{
            ...
            }, FUNC(deployAIRecursive), _this] call CFUNC(waitUntilAndExecute);
        }, [_vehicle, _unitsToDeploy], 1] call CFUNC(waitAndExecute);
    } else {
        [{
        ...
        }, _vehicle] call CFUNC(waitUntilAndExecute);
    };
};

[FUNC(deployAIRecursive), [_vehicle, _unitsToDeploy], _deployTime + 4] call CFUNC(waitAndExecute);
