...
class CfgAmmo {
    ...
    class SatchelCharge_Remote_Ammo: PipeBombBase {
        ...
        ACE_explosives_ringtones[] = {{"\z\addons\explosives\Data\Audio\Cellphone_Ring.wss",0.75}};  // (Optional) Defines IED ringtone(s) to be randomly selected upon detonation by cellphone, each entry includes a sound file path and its duration in seconds. Should be in array of [path,duration]
    };
};
```
