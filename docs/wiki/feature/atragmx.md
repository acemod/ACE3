...
- Verify that the set `C1 Coefficient` is that of the desired ammunition:
  - *It can be viewed in the ACE Arsenal as a magazine stat, or with a Config Viewer.*
...
- Input the `C1 coefficient` of the bullet, which can be found:
  - in the ACE Arsenal, as a stat on a given magazine
  - with the Eden Editor's or Debug Console's `Config Viewer`:
